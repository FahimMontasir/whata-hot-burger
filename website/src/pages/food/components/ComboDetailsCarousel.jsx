...
export default function ProductDetailsCarousel({ combo }) {
  ...
  const settings2 = {
    ...
    variableWidth: true,
    centerPadding: "0px",
    slidesToShow: combo.photoUrls.length > 3 ? 3 : combo.photoUrls.length,
  };
    ...
    <RootStyle>
      <Box sx={{ p: 1 }}>
        ...
        >
          <Slider {...settings1} asNavFor={nav2} ref={slider1}>
            {combo.photoUrls.map((item) => (
              <LargeItem key={item} item={item} />
            ))}
          </Slider>
          <CarouselControlsArrowsIndex
            index={currentIndex}
            total={combo.photoUrls.length}
            onNext={handleNext}
      ...
      <Box
        sx={{
          ...
          ...(combo.photoUrls.length === 1 && {
            maxWidth: THUMB_SIZE * 1 + 16,
          }),
          ...(combo.photoUrls.length === 2 && {
            maxWidth: THUMB_SIZE * 2 + 32,
          }),
          ...(combo.photoUrls.length === 3 && {
            maxWidth: THUMB_SIZE * 3 + 48,
          }),
          ...(combo.photoUrls.length === 4 && {
            maxWidth: THUMB_SIZE * 3 + 48,
          }),
          ...(combo.photoUrls.length >= 5 && { maxWidth: THUMB_SIZE * 6 }),
          ...(combo.photoUrls.length > 2 && {
            position: "relative",
            "&:before, &:after": {
              top: 0,
